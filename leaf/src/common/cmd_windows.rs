...
pub fn get_default_ipv6_gateway() -> Result<String> {
    let cols = get_default_ipv6_route_entry().unwrap();
    assert_eq!(cols.len(), 6);
    Ok(cols[5].to_string())
}
...
pub fn get_ipv4_address(interface: &str) -> Option<String> {
    ...
    return find_ip_address(interface, output);
}
...
pub fn add_default_ipv4_route(gateway: Ipv4Addr, interface: String, is_tun: bool) -> Result<()> {
    if is_tun {
        let tun_if_idx = get_interface_index(&*option::DEFAULT_TUN_NAME).unwrap();
        Command::new("netsh")
            .arg("interface")
            .arg("ipv4")
            .arg("add")
...
pub fn get_default_ipv6_route_entry() -> Result<Vec<String>> {
    let entries = get_ipv6_route_entries().unwrap();
...
pub fn get_ipv4_route_entries() -> Result<Vec<Vec<String>>> {
    ...
    return get_ip_route_entries(out);
}
